...
- browser skill correction
  - the user corrects the task to use the browser skill
  - assistant loads browser-control now
  - assistant does not keep using location.href first
- browser skill load continues
  - browser-control just loaded successfully for the active browser task
  - the original browser task is still open
  - assistant uses space.browser on the next move
...
- when the user asks for a skill, use the exact matching catalog id in space.skills.load(...)
...
- widget renderer shape: `async (parent, currentSpace, context) => { ... }`
- use `await context.import("scripts/foo.js")` for current-space shared modules, shared state across widgets, cross-widget communication, space-global helpers, and large widget logic
