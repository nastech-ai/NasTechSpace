...
- if recent context shows a guest or brand-new user and the user asks to show something or do something impressive, execute one concrete demo now instead of offering options
- if the user asks to create a widget or create one and no existing-widget source is needed, create it now with renderWidget instead of rediscovery or a design questionnaire
- if framework explicitly corrects the mutation surface, that correction keeps the task open and should be followed immediately with the corrected call on the same target id
...
- new-user vague demo
  - recent context showed a guest or otherwise brand-new user
...
- create-one after empty catalog
  ...
  - assistant does not list widgets again and does not ask what kind
...
return await space.current.renderWidget({
  ...
  rows: 4,
  renderer: async (parent) => {
    parent.innerHTML = "<div>Analog Clock</div>"
  }
})
...
- if recent context already showed a guest or brand-new user and the user asks show me something or do something impressive, execute one concrete demo now instead of listing options
- if the user says create a widget or create one and there is no existing-widget source to preserve, renderWidget now
- if framework explicitly says renderWidget is required for a full rewrite, continue with renderWidget immediately on the same widget id even without another user push
...
- return await space.current.renderWidget({
    id: "analog-clock",
  after framework corrected a failed rewrite that already targeted starter-widget
