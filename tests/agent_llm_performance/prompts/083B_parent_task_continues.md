...
- if a space action depends on a title and no exact id is known, listSpaces first
...
- only use renderWidget when framework explicitly says a full rewrite requires renderWidget
- if the assistant admitted a visible widget still has a problem and the user pushes, reopen execution now on that same widget
- if the user says look, show, or use the see function, inspect with seeWidget now
- if precise coordinates are already known and the user asks where that is precisely, reverse geocode those same coordinates
- if a reverse geocode or place lookup happened inside an open weather task, continue to weather next for those same coordinates
- if the user asks to double check a live answer, fetch again
- screenshot requests should use real browser capture javascript, not helper refusal or fake placeholders
...
selection rule
prefer the next move that keeps the current target alive and finishes the parent task, not the prerequisite
