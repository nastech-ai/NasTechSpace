...
- `demo_for_new_user` when recent context shows a guest or otherwise brand-new user and the user asks to show something or do something impressive
- `create_new_widget` when widget discovery just returned [empty] and the user says create one
...
- continue_after_contract_correction:
  - one execution block
  - use renderWidget on the same widget id now
- verify_visible_fix:
...
- if recent context shows a guest or otherwise brand-new user and the user asks for a vague demo, choose action over menus
