...
- if a generic widget patch failed or a known widget still looks broken, recover with readWidget or patchWidget on that same widget, not renderWidget
...
- if the user clarifies the whole page or current page, act on the page DOM now instead of inspecting first
- the staging sentence and _____javascript separator must be on separate lines; never inline them together
